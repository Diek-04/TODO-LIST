...
TEST(Progetto, esportazioneElenchiAttivita)
{
	...
	ASSERT_TRUE(p.esportaAttivitaSuFile(CSV, (char*)"elenco"));
	...
	ASSERT_TRUE(p.esportaAttivitaCompletateSuFile(CSV,(char *)"elenco"));
	file = fopen("elenco.csv", "r");
	if (file == NULL)
		FAIL();
	else
	...
	d.setDate(31, 8, 2017);
	ASSERT_TRUE(p.esportaAttivitaInScadenzaSuFile(d, CSV, (char*)"elenco"));
	file = fopen("elenco.csv", "r");
	if (file == NULL)
		FAIL();
	else
...
TEST_F(TestTODO, creazioneEelencoProgetti)
{
	...
	ASSERT_TRUE(p = TODO::creaProgetto((char*)"Progetto1"));
	p->getDenominazione(d);
	ASSERT_STREQ("Progetto1", d);
	delete p;
	ASSERT_FALSE(p = TODO::creaProgetto((char*)"Progetto1"));
	ASSERT_TRUE(p = TODO::creaProgetto((char*)"Progetto2"));
	p->getDenominazione(d);
	ASSERT_STREQ("Progetto2", d);
	delete p;
	ASSERT_TRUE(p = TODO::creaProgetto((char*)"Progetto3"));
	p->getDenominazione(d);
...
TEST_F(TestTODO, selezioneProgetto)
{
	...
	ASSERT_FALSE(p = TODO::selezionaProgetto((char *)"Progetto"));
	ASSERT_TRUE(p = TODO::selezionaProgetto((char *)"Progetto2"));
	p->getDenominazione(d);
	ASSERT_STREQ("Progetto2", d);
	delete p;
}
